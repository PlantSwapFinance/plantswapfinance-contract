...
To work on:
    ...
    - Add a separate reward (1-2%) of the pending split reward for whoever execute update externaly or compound or deposit (!harvest && !withdraw)
    - Check if Gardener register and has special badge/point on hompound&harvest
    - If PlantReward = StakedToken = Compound Plant
    - Make Plant reward and gStake optional (No need to set MasterGarden, Mint gToken and deposit them)
    - Add a value for a deposit fee % till block number + gardener.lastUpdate
*/
...
contract VerticalGarden is ERC20, ReentrancyGuard {
    ...
    bool public rewardTokenDifferentFromStakedToken;        // If true it mean the reward from MasterChef is not the same token than the Staked Token
    bool public stakedTokenMasterChefContractIsSmartChef;   // If true it mean the Staking Contract is a SmartChef and not a MasterChef (no pid)
    uint16 public verticalGardenStakedTokenMasterChefPid;       // The pid of the MasterChef pool for the StakedToken
    uint16 public verticalGardenMasterGardenerPid;              // The pid of the MasterGardener pool for gStakedToken
    ...
    string public gTokenSymbol;

    BEP20 public stakedToken;                               // Place here the token that will be stack
    BEP20 public rewardToken;                               // Place here the token that will be receive as reward
    StakedTokenMasterChef public stakedTokenMasterChef;     // MasterChef contract for stackedToken
    StakedTokenSmartChef public stakedTokenSmartChef;       // MasterChef contract for stackedToken
    BEP20 public plant;                            // PLANT Token
    MasterGardener public plantMasterGardener;              // Plantswap MasterGardener
    ...
    mapping(address => uint256) public compoudedReward;
    mapping(address => uint256) public harvestedReward;
    mapping(address => uint256) public harvestedPlant;
    uint256 public totalStakedToken;
    uint256 public totalPendingRewardTokenToSplit;
    uint256 public totalPendingPlantRewardToSplit;
    uint256 public totalStakedTokenEachBlock;
    uint256 public toStakeGStakedToken;
    uint256 public lastUpdBlockNumber;         // Block of the last update
    uint256 public lastUpdBlockNumberPrevious; // Block number at previous update
    uint256 public lastUpdTotalStakedToken;    // totalStakedToken at previous update
    uint256 public lastUpdRewardTokenGained;   // rewardTokenGained at previous update
    uint256 public lastUpdPlantGained;         // plantGained at previous update
    ...
    constructor(
        ...
        bool _stakedTokenMasterChefContractIsSmartChef,
        uint16 _verticalGardenStakedTokenMasterChefPid,
        uint16 _verticalGardenMasterGardenerPid,
        BEP20 _stakedToken,
        BEP20 _rewardToken,
        BEP20 _plant,
        StakedTokenMasterChef _stakedTokenMasterChef,
        StakedTokenSmartChef _stakedTokenSmartChef,
        MasterGardener _plantMasterGardener
    ) ERC20('Plantswap.finance Vertical Garden', 'vgPLANT') payable {
        depositActive = false;
        ...
        verticalGardenStakedTokenMasterChefPid = _verticalGardenStakedTokenMasterChefPid;
        verticalGardenMasterGardenerPid = _verticalGardenMasterGardenerPid;
        devAddress = msg.sender;
        depositFeeAddress = msg.sender;
        stakedToken = _stakedToken;
        rewardToken = _rewardToken;
        plant = _plant;
        stakedTokenMasterChef = _stakedTokenMasterChef;
        stakedTokenSmartChef = _stakedTokenSmartChef;
        plantMasterGardener = _plantMasterGardener;
    }
    ...
    function updateReward() internal {
        if(totalStakedToken > 0) {
            ...
            if(lastUpdBlockNumber < block.number) {
                ...
                totalPendingRewardTokenToSplit += uint256(rewardTokenGained);
                lastUpdRewardTokenGained = uint256(rewardTokenGained);
                totalStakedTokenEachBlock += uint256(totalStakedToken) * (uint256(block.number) - uint256(lastUpdBlockNumber));
                lastUpdTotalStakedToken = uint256(totalStakedToken);
                ...
                if(plantGained > 0) {
                    ...
                    lastUpdPlantGained = uint256(plantGained);
                }
                lastUpdBlockNumberPrevious = uint256(lastUpdBlockNumber);
                lastUpdBlockNumber = uint256(block.number);
            }
        }
    }

    function pendingRewardToken(address _farmer) public view returns (uint256) {
        uint256 ltotalPendingRewardTokenToSplit = totalPendingRewardTokenToSplit;
        uint256 lTotalWeight = totalWeight();
        uint256 lGardenerWeight = gardenerWeight(_farmer);
        uint256 lExpectedRewardToken = 0;
        if(ltotalPendingRewardTokenToSplit > 0 && lGardenerWeight > 0 && lTotalWeight > 0) {
            lExpectedRewardToken = ((uint256(ltotalPendingRewardTokenToSplit) * uint256(lGardenerWeight)) / uint256(lTotalWeight));
        }
    ...
    function estimateRewardToken(address _farmer) public view returns (uint256) {
        ...
        uint256 lExpectedRewardToken = 0;
        if(ltotalPendingRewardTokenToSplit > 0 && lGardenerWeight > 0 && lTotalWeight > 0) {
            lExpectedRewardToken = (((uint256(ltotalPendingRewardTokenToSplit) + uint256(lTotalPendingInMasterChef)) * uint256(lGardenerWeight)) / uint256(lTotalWeight));
        }
        return lExpectedRewardToken;
    }
    ...
    function totalWeight() public view returns (uint256) {
        ...
        uint256 llastUpdBlockNumber = lastUpdBlockNumber;
        uint256 lTotalWeight = 0;
        if(llastUpdBlockNumber > 0 && lTotalStakedToken > 0) {
            lTotalWeight = uint256(lTotalStakedTokenEachBlock) + ((uint256(block.number) - uint256(llastUpdBlockNumber)) * uint256(lTotalStakedToken));
        }
        return lTotalWeight;
    }
    ...
    function compoundReward() internal {
        ...
        if(!rewardTokenDifferentFromStakedToken) {
            ...
            if(gardener.dateLastUpdate < lastUpdBlockNumber) {
                if(totalPendingRewardTokenToSplit > 0 && gardener.balance > 0 && gardener.dateLastUpdate > 0) {
    ...
    function harvestReward() internal {
        ...
        Gardener memory gardener = gardeners[farmer];
        if(gardener.dateLastUpdate < lastUpdBlockNumber) {
            if(totalPendingRewardTokenToSplit > 0 && gardener.balance > 0) {
                uint256 expectedRewardToken = pendingRewardToken(farmer);
                uint256 expectedPlant = pendingPlantReward(farmer);
                uint256 lGardenerWeight = gardenerWeight(farmer);
    ...
    function deposit(uint256 amount) external nonReentrant gardenActive {
        ...
        if(totalStakedToken > 0) {
            updateReward();
            if(totalPendingRewardTokenToSplit > 0 || totalPendingPlantRewardToSplit > 0) {
                if(!rewardTokenDifferentFromStakedToken) {
                    compoundReward();
                } else {
                    harvestReward();
    ...
    function depositToken(uint256 amount) internal {
        ...
        gardeners[farmer] = gardener;
        if(lastUpdBlockNumber < 1) { // Initialize lastUpdBlockNumber on first deposit and stack in master gardener
